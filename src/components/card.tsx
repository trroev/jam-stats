...
export default function Card({ track, artist, show, index }: CardProps) {
  ...
  return (
    <div className="flex justify-between items-center w-full lg:justify-center relative bg-transparentDarkGray border-2 border-black lg:p-2  transition-all duration-500 delay-75">
      ...
      <div className={"w-4/5 max-w-4/5 lg:w-2/3 lg:max-w-2/3 p-2 flex flex-col " + cardHeights}>
          <div className={textDivClasses + " items-end"}>
            <a href={toDisplay?.spotifyUrl}><Image height={20} width={20} alt="Spotify logo" src={logo}/></a>
            <h2 className="truncate font-extrabold md:text-lg">{toDisplay?.name}</h2>
          </div>
      ...
      {toDisplay ? 
      <div className={"bg-black float-right transition-all duration-500 delay-75 flex " + cardHeights + " " + cardWeights}>
      <Image
